...
// display size in pixels
        ...
        Log.d("preview", "getScreenSize w:"+width + " height:" + height);
...
//        }
    ...
    private Size getOptimalPreviewSize(List<Size> sizes, int w, int h) {
        ...
        Log.d(TAG, "optimal width:"+optimalSize.width + " height:"+optimalSize.height);
        return optimalSize;
    }
...
//        int rotate = (info.orientation - degrees + 360) % 360;
        ...
        public void onPreviewFrame(byte[] data, Camera camera)
        {
            ...
            try {
                ...
                if (result != 0 && !barcodeScanned)
                {
                    ...
                    for (Symbol sym : syms)
                    {
                        if(callbackQRcode != null)
                            callbackQRcode.qrCodeDetected(sym.getData());
                        Log.d(TAG, " getData)" + sym.getData());
                        break;
                    }
                }
            } catch (Exception e) {
    ...
    public void setCallbackQRcode(CallbackQRcode callbackQRcode) {
        this.callbackQRcode = callbackQRcode;
    }
}